...
static DEVICE char linear_mem_pool[Config::MaxGPUThreads*Config::ThreadLocalTemporaryHeapSizeGPU]={0};
static DEVICE size_t alloc_head[Config::MaxGPUThreads]={0};
...
template<typename T> inline DEVICE T* AllocateTemporaryBuffer(size_t N, int tidx){
    ...
    char* res=&(linear_mem_pool[ tidx*Config::ThreadLocalTemporaryHeapSizeGPU + head0 ]);
...
template<typename T> inline DEVICE  int FreeTemporaryBuffer(const T* ptr, int tidx){
    ...
    std::ptrdiff_t head=reinterpret_cast<std::ptrdiff_t>(ptr) - reinterpret_cast<std::ptrdiff_t>(&linear_mem_pool[tidx* Config::ThreadLocalTemporaryHeapSizeGPU]);
